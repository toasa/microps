...
extern int ip_endpoint_pton(const char *src, struct ip_endpoint *dst);
extern char *ip_endpoint_ntop(const struct ip_endpoint *src, char *dst,
                              size_t size);
...
extern int ip_proto_register(uint8_t type, ip_proto_handler_t handler);
