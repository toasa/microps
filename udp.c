...
#define UDP_SRC_PORT_MIN 49152
#define UDP_SRC_PORT_MAX 65535
...
ssize_t udp_sendto(int id, uint8_t *data, size_t len,
    ...
    if (local.addr == IP_ADDR_ANY) {
        struct ip_iface *iface = ip_route_get_iface(foreign->addr);
        if (!iface) {
    ...
    if (!local.port) {
        for (uint32_t p = UDP_SRC_PORT_MIN; p <= UDP_SRC_PORT_MAX; p++) {
